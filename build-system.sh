...
#          "gcc"      use GCC toolchain, default it nothing is set.
#          "clang"    use LLVM/clang toolchain.
#          "axivion"  use Axivion suide to run code analysis.
#
#    ENABLE_ANALYSIS=ON
...
#       is running the analysis with the Axivion suite.
#
#
#-------------------------------------------------------------------------------
...
case "${TOOLCHAIN}" in
    gcc)
        ...
        # Luckily, CROSS_COMPILER_PREFIX can be build from TRIPLE by just adding
        # a dash.
        CMAKE_PARAMS+=( -D CROSS_COMPILER_PREFIX=${TRIPLE}- )
        ;;

    clang)
        TOOLCHAIN_FILE="${OS_SDK_PATH}/sdk-sel4-camkes/kernel/llvm.cmake"
        CMAKE_PARAMS+=( -D TRIPLE=${TRIPLE} )
        ;;
